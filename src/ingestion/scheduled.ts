...
import { parseCurationsYaml } from '../curations/config';
...
export async function runScheduledIngestion(input: ScheduledIngestionInput): Promise<ScheduledIngestionResult> {
  ...
  const curationsInserted =
    input.kind === 'daily' && input.curationsYaml
      ? (await replaceConfiguredCurations(input.db, parseCurationsYaml(input.curationsYaml))).inserted
      : 0;
